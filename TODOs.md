...
7. Resistance mode with the Saris H3 still isn't quite right -- switching to resistance mode provides a VERY HIGH resistance that doesn't seem to change when resistance is modified through the UI. It's certainly not the same "33%" default that TrainerRoad provides on the same trainer. When I look at what the UI returns for the trainer's characteristics, it says the supported resistance range is (0,0), so something seems to not be getting interpreted correctly.
8. **Interval transition audio alerts.** Play an audible cue shortly before and at the moment of each interval change, so users don't need to watch the screen during hard efforts.
...
9. **Per-interval breakdown in the post-workout summary.** Append a scrollable table to `WorkoutSummaryDialog` showing how the user performed against each interval target - the key "did I hit my numbers?" question that structured training demands.
