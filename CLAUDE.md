...
## Versioning
The app version follows `MAJOR.MINOR.PATCH` (starting at `0.1.0`) and is displayed at the bottom of the Settings tab. It is defined in two places that must be kept in sync: `pyproject.toml` (`[project].version`) and `opencycletrainer/__init__.py` (`__version__`). When making a change, bump the version accordingly:
- **PATCH** (subminor/build) — increment on every bug fix or performance improvement (i.e. TODO list item that is a correction or minor tweak, or a direct request for the same).
- **MINOR** — increment when a new feature is added (reset PATCH to 0) (i.e. TODO item or request that is a brand new function that didn't exist before).
- **MAJOR** — increment only when explicitly specified (reset MINOR and PATCH to 0).
